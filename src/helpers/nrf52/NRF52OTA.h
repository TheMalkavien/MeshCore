#pragma once

// Opt-in: only built when the target asks for mesh OTA over LoRa with
// -D MESH_LORA_OTA and a linker script that reserves a staging area (see
// boards/nrf52840_s140_v6_lora_ota.ld). Without it 'start ota' keeps its stock
// nRF52 behavior (BLE DFU advertising) and none of this is linked in.
#if defined(NRF52_PLATFORM) && defined(MESH_LORA_OTA)

#include <stddef.h>
#include <stdint.h>

// Mesh OTA controller for nRF52840 targets. Speaks the exact same text and
// binary command protocol as RP2040OTAController and ESP32OTAController
// (start/begin/write/end/status/abort), so the web OTA tool drives all three
// platforms identically.
//
// What differs, and why: the ESP32 inflates the gzip stream as it arrives and
// writes straight into the spare app partition. The nRF52 has no spare
// partition - the app region is the one it is executing from - so the stream is
// stored verbatim in the staging area and only decompressed at the next boot,
// by NRF52OTAApply.cpp. Consequences visible from the protocol:
//   - gzip is mandatory, not opportunistic: a raw image does not fit alongside
//     the running one (see the layout comment in the linker script)
//   - 'end' costs several seconds: the image is fully decompressed with a
//     throwaway sink before the update is armed, so a bad image is reported to
//     the client instead of being discovered at the next boot
class NRF52OTAController {
public:
  enum BinaryOpcode : uint8_t {
    BIN_OP_START  = 1,
    BIN_OP_BEGIN  = 2,
    BIN_OP_WRITE  = 3,
    BIN_OP_END    = 4,
    BIN_OP_STATUS = 5,
    BIN_OP_ABORT  = 6
  };

  NRF52OTAController();

  bool startSession(const char *id, char reply[]);
  bool handleCommand(const char *command, char reply[]);
  bool handleBinaryCommand(uint8_t opcode, const uint8_t *payload, size_t payload_len, char reply[]);
  bool isSleepInhibited() const;

private:
  // Received segments inside the current staging block, sorted and disjoint
  // (block-relative offsets, end exclusive). Out-of-order chunks land here so
  // one lost chunk no longer invalidates the rest of its batch: the holes are
  // advertised via 'miss=' in the status reply and only they get resent.
  struct StageRange {
    uint16_t start;
    uint16_t end;
  };
  static const int MAX_STAGE_RANGES = 24;

  bool _armed;
  bool _active;
  bool _stream_started;   // first bytes seen, gzip magic checked
  char _md5[33];          // expected MD5 of the received byte stream ("" = none)
  size_t _expected_size;
  size_t _received_size;  // contiguous prefix received (the 'done' of status replies)
  size_t _flashed_size;   // bytes already committed to the staging area
  size_t _next_progress_log;
  uint16_t _ack_every_chunks;
  uint16_t _chunks_since_ack;
  uint32_t _last_activity_millis;
  uint32_t _awaiting_reboot_since;  // image staged, waiting for the reboot command (0 = no)
  uint8_t *_stage;
  size_t _stage_size;
  size_t _stage_base;     // absolute stream offset of _stage[0] (== bytes already flushed)
  StageRange _stage_ranges[MAX_STAGE_RANGES];
  uint8_t _stage_range_count;
  void *_md5_ctx;         // ota_md5_ctx, allocated for the duration of a session

  static const char *skipSpaces(const char *p);
  static bool decodeHex(const char *hex, size_t hex_len, uint8_t *out, size_t max_out, size_t *out_len);
  static bool isValidHexChar(char c);
  bool writeChunk(size_t offset, const uint8_t *chunk, size_t chunk_len, char reply[]);
  bool writePayload(const uint8_t *data, size_t len, char reply[]);
  bool flushStage(char reply[]);
  size_t flushBlockBytes() const;
  size_t stageExtent() const;
  size_t stagePrefix() const;
  bool insertStageRange(size_t start, size_t end);
  void appendMissList(char reply[], size_t max_len) const;
  bool armRecord(char reply[]);
  void abortUpdate();
  bool sessionExpired() const;
  void expireIfIdle();
  void clearState();
};

#endif
