// RFC 1321 MD5. Compact reference implementation; see the header for why the
// nRF52 OTA path needs its own.
...
static uint32_t md5_le32(const uint8_t *p) {
  return (uint32_t) p[0] | ((uint32_t) p[1] << 8) | ((uint32_t) p[2] << 16) | ((uint32_t) p[3] << 24);
}

static void md5_transform(uint32_t state[4], const uint8_t block[64]) {
  uint32_t x[16];
  for (int i = 0; i < 16; i++) x[i] = md5_le32(block + i * 4);
...
void ota_md5_final(ota_md5_ctx *ctx, uint8_t digest[16]) {
  ...
  for (int i = 0; i < 8; i++) {
    length[i] = (uint8_t)((ctx->count[i >> 2] >> ((i & 3) << 3)) & 0xFF);
  }

  uint32_t index = (ctx->count[0] >> 3) & 0x3F;
  size_t pad_len = (index < 56) ? (56 - index) : (120 - index);
  padding[0] = 0x80;
  for (size_t i = 1; i < pad_len; i++) padding[i] = 0;
  ota_md5_update(ctx, padding, pad_len);
  ota_md5_update(ctx, length, 8);
